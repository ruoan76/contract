...
- **P1**：页面主流程可用  
- **P2**：增强/筛选/统计  
- 状态：`⬜ 未测` `🟡 部分` `✅ 通过`（🟡/✅ 基于后端集成测 IT-01~10，非前端实机联调）
...
| contracts | 合同列表 | `GET /contracts` | P0 | ✅ |
...
| Demo | 步骤 | API 链 | 状态 |
|------|------|--------|------|
| DEMO-01 | 1–5 | create → submit → approve → review → seal | ✅ |
| DEMO-02 | 1–4 | ai-review → opinions×3 → archive | ✅ |
| DEMO-03 | 1–2 | thresholds → approval history | ✅ |
| DEMO-04 | 1–2 | counterparties blacklist → create 拒绝 | ✅ |
| DEMO-05 | 1–2 | review return → revision → ai-review | ✅ |
