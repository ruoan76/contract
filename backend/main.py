...
async def lifespan(app: FastAPI):
    ...
    print(
        f"🤖 AI 审查: mock={settings.AI_REVIEW_MOCK} "
        f"sync={settings.AI_REVIEW_SYNC} base={settings.AI_BASE_URL}"
    )

    if not settings.AI_REVIEW_MOCK:
        from app.services.ai_review.mlx_health import check_mlx_reachable

        mlx_ok, mlx_msg = await check_mlx_reachable()
        if mlx_ok:
            print("✅ MLX 推理服务可达")
        else:
            print(f"⚠️  MLX 不可达（{mlx_msg}），AI 审查将返回 503")
        ...
        try:
            ...
            try:
                ...
                async with async_session() as sess:
                    ver = await refresh_config_cache(sess)
                    if ver:
                        print(f"✅ AI 审查配置缓存已加载: {ver}")
            except Exception as cache_err:
                print(f"⚠️  AI 配置缓存加载跳过: {cache_err}")
...
app.include_router(ai_review_config.router, prefix="/api/v1/ai-review", tags=["AI 审查-配置"])
