...
function switchPage(pageId) {
  ...
  const pageMap = {
    ...
    'approval-history': 11, 'seal': 12, 'archives': 13,
    'counterparties': 14, 'config': 15, 'users': 16, 'audit': 17,
    'contract-detail': -1, 'revision-workspace': -1
  };
...
// 合同详情
function showContractDetail(no) {
...
function submitContract() {
  ...
  const flowType = amountNum >= 1000000 ? 'special' : amountNum >= 100000 ? 'standard' : 'simple';
...
function returnToDrafter() {
  ...
  setTimeout(() => switchPage('revision-workspace'), 1000);
}
...
function confirmAIReview() {
  const panels = document.querySelectorAll('#stage-ai .review-panel').length;
  const handled = document.querySelectorAll('#stage-ai .review-panel .finding-handled').length;
  if (handled < panels) { showToast('⚠️ 还有 ' + (panels - handled) + ' 项 AI 标记未处理'); return; }
  showToast('✅ 审查结论已确认，进入评审工作台');
  document.querySelector('[data-stage=legal]').classList.add('completed');
  setTimeout(() => switchStage('legal'), 800);
}
...
function escalateConflict(conflictId) {
  ...
  const card = event.target.closest('.conflict-card');
  if (card) { const s = card.querySelector('.conflict-status'); if(s){s.textContent='仲裁中';s.style.color='var(--warning)';} }
  event.target.disabled = true; event.target.textContent = '已升级';
}
...
function submitRevision() {
  const opinion = document.querySelector('#page-revision-workspace textarea');
  if (opinion && !opinion.value.trim()) { showToast('⚠️ 请填写修订说明'); opinion.focus(); return; }
  showToast('✅ 修订已提交，进入重新评审流程');
  localStorage.setItem('revision-submitted', JSON.stringify({contractNo:'CTR-20240115-0001',submittedAt:new Date().toLocaleString('zh-CN'),status:'重新评审中'}));
  setTimeout(() => switchPage('review-center'), 1000);
}

// ====== 用印确认 ======
function confirmSealAction(contractId) {
  ...
  const c = JSON.parse(localStorage.getItem('contract-'+contractId)||'{}');
  c.sealStatus='completed'; c.sealedAt=new Date().toLocaleString('zh-CN');
  localStorage.setItem('contract-'+contractId, JSON.stringify(c));
}
...
function openMessageDetail(el) {
  ...
  // Navigate to the relevant page based on message type
  const typeRoutes = {
    'approval': 'approvals',
    'ai': 'ai-review',
    'system': 'approvals',
  ...
  const targetPage = typeRoutes[msgType] || 'contracts';
  showToast('📄 跳转到 ' + contractNo);
  setTimeout(() => switchPage(targetPage), 500);
}
...
function createCounterparty() {
  const name = document.querySelector('#modal-add-counterparty input[name=name]')?.value;
  if (!name?.trim()) { showToast('⚠️ 请填写相对方名称'); return; }
  hideModal('add-counterparty');
  showToast('✅ 相对方创建成功');
  const cp = {id: Date.now(), name, type: document.querySelector('#modal-add-counterparty select')?.value||'企业', credit: 'A', status: '正常', createdAt: new Date().toLocaleString('zh-CN')};
...
function filterCompare(status) {
  document.querySelectorAll('#page-clause-compare .compare-nav-btn').forEach(b => b.classList.remove('active'));
  event.target.classList.add('active');
...
function switchRole(role) {
  ...
  if (currentPage && !visible.includes(currentPage)) {
    switchPage('dashboard');
  }
  // 更新新建合同按钮
  const createBtn = document.querySelector('.header-btn[onclick*="create"]');
  if (createBtn) createBtn.style.display = visible.includes('create') ? '' : 'none';
}
...
function filterReviewReport() {
  ...
  items.forEach(item => {
    const itemLevel = item.classList.contains('high') ? 'high' : item.classList.contains('medium') ? 'medium' : 'low';
    ...
    let matchLevel = !level || itemLevel === level;
    let matchType = !type;
...
function resetReviewFilter() {
  document.getElementById('review-risk-level').value = '';
  document.getElementById('review-risk-type').value = '';
...
function loadReviewHistory(contractNo) {
  ...
  event.target.closest('.review-history-contract-item').classList.add('active');
  event.target.closest('.review-history-contract-item').style.background = 'rgba(37,99,235,0.05)';
  event.target.closest('.review-history-contract-item').style.borderColor = 'var(--primary-light)';
